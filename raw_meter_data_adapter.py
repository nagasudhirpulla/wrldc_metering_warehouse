...
class RawMeterDataAdapter:
    ...
    def pushRawCumData(self, cumData):
        ...
        dataInsertionTuple = (cumData.data_time.strftime('%Y-%m-%d %H:%M:%S'), cumData.meter_id, float(
            cumData.cumm_active_energy_wh), float(cumData.cumm_reactive_energy_high_wh), float(cumData.cumm_reactive_energy_low_wh))
        dataText = cur.mogrify(
            '(%s,%s,%s,%s,%s)', dataInsertionTuple).decode("utf-8")
        ...
        while rowIter < numRows:
            ...
            for insRowIter in range(rowIter, iteratorEndVal):
                ...
                dataInsertionTuple = (dataRow.data_time.strftime(
                    '%Y-%m-%d %H:%M:%S'), dataRow.meter_id, dataRow.freq_code, float(dataRow.act_en_wh))
    ...
    def getMeterRawBlksDataFromDb(meterId, fromTime, toTime):
        ...
        conn = psycopg2.connect(host=warehouseConfigDict['db_host'], dbname=warehouseConfigDict['db_name'],
                                user=warehouseConfigDict['db_username'], password=warehouseConfigDict['db_password'])
        sql = "select * from public.raw_meter_data where meter_id = '{0}' and data_time between '{1}' and '{2}'".format(
            meterId, fromTime.strftime('%Y-%m-%d %H:%M:%S'), toTime.strftime('%Y-%m-%d %H:%M:%S'))
